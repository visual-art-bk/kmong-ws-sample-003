...
class WebBrowser:
    ...
    async def __aenter__(self):
        ...
        try:
            ...
            browser_options = WebBrowser._get_browser_options()

            self.driver = await async_playwright().start()
            self.browser = await self.driver.chromium.launch(**browser_options)
            self.context = await self.browser.new_context()

            await self._set_context_props()
            ...
            err_msg = f"WebBrowser의 인스턴스를 설정하는 중 예외발생"

            WebBrowser._log(err_msg=err_msg)
    ...
    async def _set_context_props(self):
    ...
    async def goto(self, url: str, timeout: int = 60):
    ...
    def _get_browser_options():
        try:
            headless_mode = False
            executable_path = WebBrowser._get_browser_path()
